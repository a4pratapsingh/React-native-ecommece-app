      ...
      <ScrollView contentContainerStyle={{paddingBottom:100}}>
        ...
        <View className='relative h-[450px] bg-gray-100 mb-6'>
            ...
            <View className='absolute top-12 left-4 right-4 flex-row justify-between items-center z-10'>
                ...
                bg-white/80 rounded-full items-center justify-center'>
                    <Ionicons name='arrow-back' size={24} color={COLORS.primary}/>
                </TouchableOpacity>

                <TouchableOpacity onPress={()=> toggleWishlist(product)} className='w-10 h-10 
                bg-white/80 rounded-full items-center justify-center'>
                    ...
                    {product.sizes.map((size)=>(
                        <TouchableOpacity key={size} onPress={()=>
                            ...
                            ${selectedSize === size ? 'bg-primary border-primary' : 'bg-white border-gray-100'}`}>
                                <Text className={`text-sm font-medium ${selectedSize === size ?
                                    'text-white' : 'text-primary'}`}>{size}</Text>
                        </TouchableOpacity>
                    ))}
                </View>
                </>
