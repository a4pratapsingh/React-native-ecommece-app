...
export default function Checkout() {
    ...
    const [paymentMethod, setPaymenMethod] = useState<"cash" | "stripe">('cash')
    ...
    <SafeAreaView className='flex-1 bg-surface' edges={['top']}>
        ...
        <ScrollView className='flex-1 px-4 mt-4'>
            ...
            <Text className='text-lg font-bold text-primary mb-4'>Payment Method</Text>
             ...
             <TouchableOpacity onPress={()=>setPaymenMethod('cash')}
             className={`bg-white p-4 rounded-xl mb-4 shadow-sm flex-row items-center
                border-2 ${paymentMethod === 'cash' ? 'border-primary' : 'border-transparent'}`}>
                <Ionicons name='cash-outline' size={24} color={COLORS.primary} className='mr-3'/>
                <View className='ml3 flex-1'>
             ...
             {/* Stripe Option */}
             <TouchableOpacity onPress={()=>setPaymenMethod('stripe')}
             className={`bg-white p-4 rounded-xl mb-4 shadow-sm flex-row items-center
                border-2 ${paymentMethod === 'stripe' ? 'border-primary' : 'border-transparent'}`}>
